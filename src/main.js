/* =====================================================================
   ...
   ===================================================================== */
import { state, centreCase, loadData, sauvegardeExiste, chargerSauvegarde, effacerSauvegarde } from './state.js';
...
function etatVide(){
  ...
  state.hpCruiser=state.HP_MAX;state.score=0;state.phase='attente';state.selection=null;state.vague=1;state.actionFaite=false;state.modeTourelle=false;state.hangar=null;state.tirsGratuits=0;
...
function nouvellePartie(){
  ...
  const M=state.meta||{}; state.HP_MAX=Math.round(100*state.COLS/7)+(M.pvBonus||0)*10;   // méta : +PV de départ
  ...
  state.actionFaite=false; state.modeTourelle=false; state.hangar=null; state.tirsGratuits=0;
...
function reprendrePartie(){
  ...
  for(const f of d.fighters||[]) state.fighters.push(nouveauVaisseau(f.c,f.r,f.type,false)), Object.assign(state.fighters[state.fighters.length-1],{hp:f.hp,used:f.used});
