...
export const EVENEMENTS=[
 ...
 {titre:{fr:'ÉPAVE À LA DÉRIVE',en:'DRIFTING WRECK'}, desc:{fr:'Une épave flotte non loin. La fouiller ?',en:'A wreck floats nearby. Search it?'}, choix:[
   {ico:'loupe', nom:{fr:'Fouiller',en:'Search'}, desc:{fr:'Gros butin… ou embuscade',en:'Big loot… or an ambush'}, effet:()=>{ if(Math.random()<0.5){ state.ultimeJauge=Math.min(state.ultimeSeuil,state.ultimeJauge+Math.round(state.ultimeSeuil*0.5)); state.score+=8; logMsg('Butin ! +50% ultime','log-ylw'); } else { for(let k=0;k<3;k++){ const c=Math.floor(Math.random()*state.COLS); if(!aileEn(c,0)) faireAile(c,0,typeAile()); } logMsg('Embuscade !','log-red'); } }},
 ...
 {titre:{fr:'PARI RISQUÉ',en:'RISKY BET'}, desc:{fr:'Pile tu gagnes gros, face tu perds ta mise.',en:'Heads you win big, tails you lose your stake.'}, choix:[
   {ico:'de', nom:{fr:'Parier',en:'Bet'}, desc:{fr:'50% : +score & ultime',en:'50%: +score & ultimate'}, effet:()=>{ if(Math.random()<0.5){ state.score+=15; state.ultimeJauge=Math.min(state.ultimeSeuil,state.ultimeJauge+Math.round(state.ultimeSeuil*0.5)); logMsg('Gagné ! +50% ultime','log-ylw'); } else logMsg('Perdu…','log-red'); }},
 ...
 {titre:{fr:'CHAMP DE DÉBRIS',en:'DEBRIS FIELD'}, desc:{fr:'Un champ de débris bloque la route.',en:'A debris field blocks the way.'}, choix:[
   {ico:'vent', nom:{fr:'Foncer',en:'Push through'}, desc:{fr:'50% : +1 vaisseau, 50% : -15% PV',en:'50%: +1 ship, 50%: -15% HP'}, effet:()=>{ if(Math.random()<0.5){ deployerVaisseau('normal'); logMsg('Vaisseau récupéré !','log-grn'); } else { state.hpCruiser=Math.max(1,Math.round(state.hpCruiser*0.85)); logMsg('Collision !','log-red'); } }},
 ...
 {titre:{fr:'ESSAIM DE DRONES',en:'DRONE SWARM'}, desc:{fr:'Un essaim inoffensif traverse ta route.',en:'A harmless swarm crosses your path.'}, choix:[
   {ico:'loupe', nom:{fr:'Étudier',en:'Study'}, desc:{fr:'+1 amélioration, embuscade possible',en:'+1 upgrade, possible ambush'}, effet:()=>{ ameliorationAleatoire(); if(Math.random()<0.35){ for(let k=0;k<2;k++){ const c=Math.floor(Math.random()*state.COLS); if(!aileEn(c,0)) faireAile(c,0,typeAile()); } logMsg('Les drones attaquent !','log-red'); } }},
 ...
 {titre:{fr:'RECYCLAGE D\'URGENCE',en:'EMERGENCY RECYCLING'}, desc:{fr:'Convertir un vaisseau en pièces de rechange ?',en:'Convert a ship into spare parts?'}, choix:[
   {ico:'engrenage', nom:{fr:'Recycler',en:'Recycle'}, desc:{fr:'-1 vaisseau, +25% PV',en:'-1 ship, +25% HP'}, effet:()=>{ if(state.fighters.length>1){ state.fighters.pop(); state.hpCruiser=Math.min(state.HP_MAX,state.hpCruiser+Math.round(state.HP_MAX*0.25)); logMsg('Vaisseau recyclé','log-grn'); } else logMsg('Pas assez de vaisseaux','log-red'); }},
 ...
 {titre:{fr:'ANOMALIE GRAVITATIONNELLE',en:'GRAVITATIONAL ANOMALY'}, desc:{fr:'Une distorsion étrange affecte la zone.',en:'A strange distortion affects the area.'}, choix:[
   {ico:'gel', nom:{fr:'Étudier',en:'Study'}, desc:{fr:'+50% ultime, risque de perdre un vaisseau',en:'+50% ultimate, risk of losing a ship'}, effet:()=>{ state.ultimeJauge=Math.min(state.ultimeSeuil,state.ultimeJauge+Math.round(state.ultimeSeuil*0.5)); if(state.fighters.length>1){ state.fighters.pop(); logMsg('Un vaisseau aspiré par la faille !','log-red'); } }},
 ...
 {titre:{fr:'CONTRAT DE MERCENAIRES',en:'MERCENARY CONTRACT'}, desc:{fr:'Des mercenaires proposent leurs services.',en:'Mercenaries offer their services.'}, choix:[
   {ico:'de', nom:{fr:'Engager (5 cristaux)',en:'Hire (5 crystals)'}, desc:{fr:'+1 cuirassé',en:'+1 battleship'}, effet:()=>{ if((state.meta.cristaux||0)>=5){ state.meta.cristaux-=5; deployerVaisseau('bouclier'); } else logMsg('Pas assez de cristaux','log-red'); }},
 ...
 {titre:{fr:'CHAMP DE MINES DÉSAMORCÉ',en:'DEFUSED MINEFIELD'}, desc:{fr:'Les mines semblent inertes… ou pas.',en:'The mines seem inert… or maybe not.'}, choix:[
   {ico:'loupe', nom:{fr:'Récupérer les explosifs',en:'Salvage the explosives'}, desc:{fr:'+1 tir gratuit, 30% : -10% PV',en:'+1 free shot, 30%: -10% HP'}, effet:()=>{ state.tirsGratuits=(state.tirsGratuits||0)+1; if(Math.random()<0.3){ state.hpCruiser=Math.max(1,Math.round(state.hpCruiser*0.9)); logMsg('Une mine explose !','log-red'); } }},
 ...
 {titre:{fr:'BALISE ENNEMIE',en:'ENEMY BEACON'}, desc:{fr:'Une balise attire l\'attention ennemie… ou brouille leurs senseurs.',en:'A beacon attracts enemy attention… or jams their sensors.'}, choix:[
   {ico:'demon', nom:{fr:'Détruire',en:'Destroy'}, desc:{fr:'+8 score, embuscade',en:'+8 score, ambush'}, effet:()=>{ state.score+=8; for(let k=0;k<3;k++){ const c=Math.floor(Math.random()*state.COLS); if(!aileEn(c,0)) faireAile(c,0,typeAile()); } logMsg('Embuscade !','log-red'); }},
 ...
 {titre:{fr:'RITUEL DE L\'ÉQUIPAGE',en:'CREW RITUAL'}, desc:{fr:'L\'équipage propose un pari amical avant la bataille.',en:'The crew offers a friendly bet before the battle.'}, choix:[
   {ico:'de', nom:{fr:'Parier gros',en:'Bet big'}, desc:{fr:'50% : +20 score, 50% : -10% PV',en:'50%: +20 score, 50%: -10% HP'}, effet:()=>{ if(Math.random()<0.5){ state.score+=20; logMsg('Pari gagné !','log-ylw'); } else { state.hpCruiser=Math.max(1,Math.round(state.hpCruiser*0.9)); logMsg('Pari perdu…','log-red'); } }},
 ...
 {titre:{fr:'OFFRE DU CONSTRUCTEUR',en:'BUILDER\'S OFFER'}, desc:{fr:'Un constructeur propose un vaisseau d\'occasion.',en:'A builder offers a used ship.'}, choix:[
   {ico:'vaisseau', nom:{fr:'Acheter (5 cristaux)',en:'Buy (5 crystals)'}, desc:{fr:'+1 tireur',en:'+1 sniper'}, effet:()=>{ if((state.meta.cristaux||0)>=5){ state.meta.cristaux-=5; deployerVaisseau('sniper'); } else logMsg('Pas assez de cristaux','log-red'); }},
   {ico:'refuser', nom:{fr:'Refuser',en:'Refuse'}, desc:{fr:'Rien',en:'Nothing'}, effet:()=>{}} ]},
 ...
 {titre:{fr:'DUEL D\'HONNEUR',en:'DUEL OF HONOR'}, desc:{fr:'Un pilote solitaire propose un duel amical.',en:'A lone pilot offers a friendly duel.'}, choix:[
   {ico:'epee', nom:{fr:'Accepter',en:'Accept'}, desc:{fr:'+5 grade à un vaisseau au hasard',en:'+5 rank to a random ship'}, effet:()=>{ if(state.fighters.length){ const f=state.fighters[Math.floor(Math.random()*state.fighters.length)]; f.kills=(f.kills||0)+5; logMsg('Vaisseau aguerri !','log-grn'); } }},
 ...
 {titre:{fr:'VAISSEAU FANTÔME',en:'GHOST SHIP'}, desc:{fr:'Un vaisseau dérive, sans équipage.',en:'A ship drifts, with no crew.'}, choix:[
   {ico:'loupe', nom:{fr:'Investiguer',en:'Investigate'}, desc:{fr:'50% : +1 cuirassé, 50% : embuscade',en:'50%: +1 battleship, 50%: ambush'}, effet:()=>{ if(Math.random()<0.5){ deployerVaisseau('bouclier'); logMsg('Cuirassé récupéré !','log-grn'); } else { for(let k=0;k<2;k++){ const c=Math.floor(Math.random()*state.COLS); if(!aileEn(c,0)) faireAile(c,0,typeAile()); } logMsg('C\'était un piège !','log-red'); } }},
   {ico:'refuser', nom:{fr:'Ignorer',en:'Ignore'}, desc:{fr:'Rien',en:'Nothing'}, effet:()=>{}} ]},
 ...
 {titre:{fr:'MARCHÉ NOIR',en:'BLACK MARKET'}, desc:{fr:'Un marché clandestin propose des upgrades... à prix fort.',en:'A clandestine market offers upgrades... at a steep price.'}, choix:[
   {ico:'panier', nom:{fr:'Acheter (8 cristaux)',en:'Buy (8 crystals)'}, desc:{fr:'+2 améliorations',en:'+2 upgrades'}, effet:()=>{ if((state.meta.cristaux||0)>=8){ state.meta.cristaux-=8; ameliorationAleatoire(); ameliorationAleatoire(); } else logMsg('Pas assez de cristaux','log-red'); }},
